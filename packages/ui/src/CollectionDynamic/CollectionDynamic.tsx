...
import type {
  CollectionDynamicProps,
  CollectionDynamicOwnerState
} from './CollectionDynamic.types';

const CollectionDynamic = (props: CollectionDynamicProps) => {
  const [showResults, setShowResults] = React.useState(!props?.algoliaSettings?.forceMinCharacters);
  const [allResulsLink, setAllResulsLink] = React.useState('');
  ...
  const cbOnChange = (isValid: boolean, query?: string) => {
    ...
    if (isValid) {
      setAllResulsLink(`/search?query=${query}`);
    } else {
      setAllResulsLink('');
    }
  };
  ...
  return (
    <ErrorBoundary>
      <Root
        ...
        data-testid={`CollectionDynamic-${variant}`}>
        ...
        <AlgoliaSearch algoliaSettings={algoliaSettings} algoliaSearchKey={algoliaSearchKey}>
          <ContentGrid ownerState={ownerState}>
            <FiltersOuterWrap ownerState={ownerState}>
              ...
              (!!algoliaSettings?.showFilters || !!algoliaSettings?.showSearchBox) ? (
                <FiltersWrap ownerState={ownerState}>
                  {!!algoliaSettings?.indexName && !!algoliaSettings?.showSearchBox ? (
                    <SearchBoxWrap ownerState={ownerState}>
                      <SearchBox
                        ownerState={ownerState}
                        searchAsYouType={!!algoliaSettings?.searchAsYouType}
                        forceMinCharacters={algoliaSettings?.forceMinCharacters}
                        minCharacters={algoliaSettings?.minCharacters}
            ...
            <ResultsWrap ownerState={ownerState}>
              <ItemsGrid ownerState={ownerState}>
                {showResults &&
                !!algoliaSettings?.indexName &&
                !!algoliaSettings?.useInfiniteHits ? (
            ...
            {!!algoliaSettings?.showViewAll && showResults && allResulsLink !== '' && (
              <ViewAllResultsWrap ownerState={ownerState}>
                <ContentModule
                  __typename="Link"
...
const ViewAllResultsWrap = styled(Box, {
  name: 'CollectionDynamic',
  slot: 'ViewAllResultsWrap',
  overridesResolver: (_, styles) => [styles.viewAllResultsWrap]
})<{ ownerState: CollectionDynamicOwnerState }>``;
