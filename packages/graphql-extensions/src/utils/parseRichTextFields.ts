...
const collectLinks = (node: any) => {
  ...
  for (var i = 0; i < content?.length; i++) {
    if (
      content[i]?.data?.target?.sys?.type === 'Link' &&
      content[i]?.data?.target?.sys?.linkType === 'Entry'
    ) {
...
async function parseEntry(entry: any, ctx: ApolloContext) {
  ...
  switch (entry.sys.contentType.sys.id) {
    case 'section': {
      const contentIds = (getLocalizedField(entry.fields, 'contents', ctx) || []).map(
        (c: any) => c.sys.id
      );
...
async function parseRichTextField(
  ...
  const entries = await ctx.loaders.entryLoader.loadMany(
    entriesLinks.map((id) => ({ id, preview: !!ctx.preview }))
  );
