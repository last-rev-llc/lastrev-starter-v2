...
const bodyXSS = new FilterXSS({
  whiteList: {
    ...ALLOWED_TAGS.reduce((accum, key) => ({ ...accum, [key]: ALLOWED_ATTRIBUTES }), {})
  },
    ...
    RichText: {
      ...
      links: async (raw: any, _args: any, ctx: ApolloContext) => {
        ...
        const [entries, assets] = await Promise.all([
          ...
          ctx.loaders.assetLoader.loadMany(
            Array.from(assetsLinks.keys()).map((id) => ({ id, preview: !!ctx.preview }))
          )
        ]);

        // Loop through entries and if included in hyperlinks will change type to a "Link" to utilize mappers for that type
        const tidiedEntries = await entries
          .filter(Boolean)
          .map(async (entry: Entry<any> | Error | null) => {
