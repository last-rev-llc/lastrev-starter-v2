...
const COLLECTION_DYNAMIC_ITEM_TYPES = ['Card', 'Link'];
...
export const mappers: Mappers = {
  CollectionDynamic: {
    CollectionDynamic: {
      ...
      items: async (collectionOrArray: any, args: any, ctx: ApolloContext) => {
        if (Array.isArray(collectionOrArray)) {
          return collectionOrArray;
        }
        const collection = collectionOrArray;

        const items = getLocalizedField(collection.fields, 'items', ctx) ?? [];
